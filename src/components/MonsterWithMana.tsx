...
interface MonsterWithManaProps {
  ...
  isDroppable?: boolean;
  idPrefix?: string;
  hideEmptySlotVisual?: boolean; // バトル画面本体でのみtrueにする
}
...
export const MonsterWithMana: React.FC<MonsterWithManaProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* 2. 各装備スロットの配置 */}
      {/* 表面(isFlipped===false)はスロット位置が存在しないデザインのため、
    裏面表示中(isFlipped===true)のときだけスロットとマナを描画する */}
      {monster.isFlipped &&
          ...
          return (
            <div key={`${monster.id}-slot-${index}`} className={slotClassName}>
              <DroppableSlot
                ...
                hideIdleBorder={hideEmptySlotVisual} // バトル画面本体では枠線を非表示にする
              >
                {mana ? (
                  /* --- 装備済みマナの描画 --- */
                  <div
                ...
                ) : hideEmptySlotVisual ? null : ( // バトル画面では空きスロットの見た目を出さない（ドロップ機能は維持）
                  /* --- 未装備スロット（点線枠）の描画 --- */
                  <div
                    style={{
                      width: '100%',
